...
public class RestInterceptor implements MethodInterceptor {
    ...
    private static final Logger REST_DIGEST_LOGGER = LoggerFactory.getLogger("rest-digest");
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        for (Object argument : arguments) {
            if (argument instanceof GeneralRequest) {
                args = JSON.toJSONString(argument);
                break;
            }
        }
        REST_LOGGER.info("{},{}.{} >>>> args:{}", appName, className, methodName, args);
        ...
        try {
            ...
            REST_LOGGER.info("{},{}.{} <<<< result:{}", appName, className, methodName, JSON.toJSONString(result));
            REST_DIGEST_LOGGER.info("({},{}.{},{},{}ms)", appName, className, methodName, hasError, diffTime);
        }
        return result;
    }
